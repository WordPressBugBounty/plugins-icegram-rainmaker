...
if ( ! class_exists( 'IG_Plugin_Usage_Tracker_V_2_0_1' ) ) {
	...
	class IG_Plugin_Usage_Tracker_V_2_0_1 {
		...
		public function __construct( $name, $text_domain, $plugin_abbr, $product_id, $plugin_plan, $plugin_file_path, $tracker_class, $allowed_by_default, $enable_on_dev = false ) {
			...
			add_action( 'rainmaker_loaded', array( $this, 'do_activation_setup' ) , 9 );
		...
		public function do_deactivation_cleanup() {
			$this->clear_scheduled_cron();
			$survey_status = isset( $_REQUEST['survey_status'] ) ? $_REQUEST['survey_status'] : null;
		...
		public function show_tracker_notice() {
			...
			$show_tracking_notice = apply_filters( $this->plugin_abbr . '_show_plugin_usage_tracking_notice', true );
